...
if sys.argv[1] == 'e':
    ...
    # Seconds taken, and size of the plaintext file encrypted.
    encryptSecSize = "Seconds: " +(str(end-start)) + "\nPlaintext size: " + (str(os.path.getsize(sys.argv[3]))) + "\n"

    # Write these vars to a file (append)
    encryptedTimingsFile = open('encryptedTiming.txt', 'a')
    encryptedTiming = encryptedTimingsFile.write(encryptSecSize)
    encryptedTimingsFile.close()
    ...
    print ("what am I xoring?:")
    print (keyLines[0])
    print (encryptedFileArrayDecs[0])
